...
KMSoptions.B            = 600; % Bootstrap repitions                       (Smaller number = faster, but more sensitive.  Recommended set B in [2000,5000].
KMSoptions.EAM_maxit    = 200;  % maximum # of iterations for EAM steps     (Program may terminate early if too small, e.g. less than 20).
KMSoptions.mbase        = [];   % Initial points in EAM is mbase*dim_p + 1  (Smaller number = faster, but more sensitive.  Recommended values in [10,30])
...
KMSoptions.EI_multi_num = [];   % Maximum number of initial points to use in multi start.  (Smaller = faster, but more sensitive.  Recommend values in [10,100]).
...
KMSoptions.EAM_thetadistort = 0.005; % Amount of distortion of theta#.  We set it equal to EAM_obj_tol in our simulations.
