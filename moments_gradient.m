...
if numgrad
    ...
    elseif  DGP == 20
        ...
        for qq = 1:dX
            ...
            Dg3b1c = -x.*normpdf(x*beta1) .*normcdf((-(x*beta2+delta2)-r*(x*beta1))/sqrt(1-r^2))*pX;
            Dg3b2c = x.*normpdf(-x*beta2-delta2).*normcdf((x*beta1-r*(-x*beta2-delta2))/sqrt(1-r^2))*pX;
            Dg3d1c = zeros(1,1);
            Dg3d2c = normpdf(-x*beta2-delta2).*normcdf((x*beta1-r*(-x*beta2-delta2))/sqrt(1-r^2))*pX;

            Dg4b1c = x.*normpdf(x*beta1+delta1).*normcdf((-(x*beta2+delta2)-r*(x*beta1+delta1))/sqrt(1-r^2))*pX ...
                -(x.*normpdf(-x*beta1-delta1).*normcdf((-x*beta2-rho*(-x*beta1-delta1))/sqrt(1-rho^2)))*pX...
                +(x.*normpdf(-x*beta1).*normcdf((-x*beta2-rho*(-x*beta1))/sqrt(1-rho^2)))*pX;

            Dg4b2c = -x.*normpdf(x*beta2+delta2).*normcdf((x*beta1+delta1-r*(-x*beta2-delta2))/sqrt(1-r^2))*pX ...
                - x.*normpdf(-x*beta2).*normcdf((-x*beta1-delta1-rho*(-x*beta2))/sqrt(1-rho^2))*pX...
                + x.*normpdf(-x*beta2).*normcdf((-x*beta1-rho*(-x*beta2))/sqrt(1-rho^2))*pX;

            Dg4d1c = normpdf(x*beta1+delta1).*normcdf((-(x*beta2+delta2)-r*(x*beta1+delta1))/sqrt(1-r^2))*pX ...
                -(normpdf(-x*beta1-delta1).*normcdf((-x*beta2-rho*(-x*beta1-delta1))/sqrt(1-rho^2)))*pX;
            Dg4d2c = -normpdf(x*beta2+delta2).*normcdf((x*beta1+delta1-r*(-x*beta2-delta2))/sqrt(1-r^2))*pX;

            % Gradients for moment equalities for entry game (See Pg 34, eq 5.1) (0,0)
            Dg1b1c = x.*normpdf(-x*beta1).*normcdf((-x*beta2-rho*(-x*beta1))/sqrt(1-rho^2))*pX;
            Dg1b2c = x.*normpdf(-x*beta2).*normcdf((-x*beta1-rho*(-x*beta2))/sqrt(1-rho^2))*pX;
            Dg1d1c = zeros(1,1);
            Dg1d2c = zeros(1,1);

            % Gradients for moment equalities for entry game (See Pg 34, eq 5.2) (1,1)
            Dg2b1c = -x.*normpdf(x*beta1+delta1).*(normcdf((x*beta2+delta2-rho*(x*beta1+delta1))/sqrt(1-rho^2)))*pX;
            Dg2b2c = -x.*normpdf(x*beta2+delta2).*(normcdf((x*beta1+delta1-rho*(x*beta2+delta2))/sqrt(1-rho^2)))*pX;
            Dg2d1c = -normpdf(x*beta1+delta1).*(normcdf((x*beta2+delta2-rho*(x*beta1+delta1))/sqrt(1-rho^2)))*pX;
            Dg2d2c = -normpdf(x*beta2+delta2).*(normcdf((x*beta1+delta1-rho*(x*beta2+delta2))/sqrt(1-rho^2)))*pX;
            ...
            %%%% Compile Moments %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            Dg_ineq((qq-1)*2 + 1,:) = [Dg3b1c Dg3b2c Dg3d1c Dg3d2c Dg3rho];
            Dg_ineq((qq-1)*2 + 2,:) = [Dg4b1c Dg4b2c Dg4d1c Dg4d2c Dg4rho];
            Dg_eq((qq-1)*2 + 1,:)   = [Dg1b1c Dg1b2c Dg1d1c Dg1d2c Dg1rho];
            Dg_eq((qq-1)*2 + 2,:)   = [Dg2b1c Dg2b2c Dg2d1c Dg2d2c Dg2rho];
        end
