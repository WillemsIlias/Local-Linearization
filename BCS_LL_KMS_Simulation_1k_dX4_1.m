...
theta_true  = [0.1;  1.1;   -0.1; -0.5;
               0.3; -0.8;      0; -0.5;
              -0.1; -0.3;   0.4];                   % True parameter vector
component   = size(theta_true,1);        % Component of theta to build confidence interval around
...
sim_hi      = 1000;      % We run simulations mm = sim_lo ... sim_hi.
% Default is sim_lo = 1 and sim_hi= Nmc
KMSoptions  = KMSoptions_BCS_Simulation();
name     = strcat('BCS_LL_component=',num2str(component),'_samplesize',num2str(n),'_dX4_sim_lo',num2str(sim_lo));
