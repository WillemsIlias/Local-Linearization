...
if flag_feas == 0 && BCS_EAM ~= 1
    [theta_feas,flag_feas,theta_init,c_init,CV_init,maxviol_init]  = KMS_2_EAM_FeasibleSearch(p,theta_0,f_ineq,f_eq,f_ineq_keep,...
        f_eq_keep,f_stdev_ineq,f_stdev_eq,G_ineq,G_eq,KMSoptions);
else
    theta_init = [];
    lambda_init= [];
    c_init     = [];
...
if ((strcmp('one-sided-UB',type) == 1 || strcmp('two-sided',type) ==1)) && flag_feas == 1
    ...
    if BCS_EAM == 1
            ...
            [lambdaU_hat,lambdaU_optbound,c,CV,EI,flagU_opt,lambdaU_feas] ...
              =  BCS_3_EAM(p,1,lambda_feas,lambda_Estep,c_Estep,CV_Estep,maxviol_Estep,lambda_init,c_init,...
                CV_init,maxviol_init,f_ineq,f_eq,f_ineq_keep,f_eq_keep,f_stdev_ineq,f_stdev_eq,G_ineq,G_eq,KMSoptions);
    ...
    elseif KMSoptions.FeasAll == 1
        for jj = 1:num_feas
            ...
            [thetaU_hat(jj,:),thetaU_optbound(jj),c(jj),CV(jj),EI(jj),flagU_opt(jj),thetaU_feas{jj}]...
                = KMS_3_EAM(p,1,theta_feas(jj,:),theta_Estep,c_Estep,CV_Estep,maxviol_Estep,theta_init,c_init,...
                    CV_init,maxviol_init,f_ineq,f_eq,f_ineq_keep,f_eq_keep,f_stdev_ineq,f_stdev_eq,G_ineq,G_eq,KMSoptions);
        end
    ...
    % Run EAM from best feasible point
        ...
        [thetaU_hat,thetaU_optbound,c,CV,EI,flagU_opt,thetaU_feas]...
            = KMS_3_EAM(p,1,theta_feas,theta_Estep,c_Estep,CV_Estep,maxviol_Estep,theta_init,c_init,CV_init,...
               maxviol_init,f_ineq,f_eq,f_ineq_keep,f_eq_keep,f_stdev_ineq,f_stdev_eq,G_ineq,G_eq,KMSoptions);
    end
...
if ((strcmp('one-sided-LB',type) == 1 || strcmp('two-sided',type) ==1)) && flag_feas == 1
   % Preset
    ...
    if BCS_EAM == 1
        ...
        =  BCS_3_EAM(-p,-1,lambda_feas,lambda_Estep,c_Estep,CV_Estep,maxviol_Estep,lambda_init,c_init,CV_init,...
        maxviol_init,f_ineq,f_eq,f_ineq_keep,f_eq_keep,f_stdev_ineq,f_stdev_eq,G_ineq,G_eq,KMSoptions);
   ...
   % Run EAM from best feasible point
        ...
        [thetaL_hat,thetaL_optbound,c,CV,EI,flagL_opt,thetaL_feas]...
            ...
            f_ineq,f_eq,f_ineq_keep,f_eq_keep,f_stdev_ineq,f_stdev_eq,G_ineq,G_eq,KMSoptions);      
    end
...
if direct_solve
    ...
    options2 =   optimoptions(@fmincon,'TolFun',toldirect2,'TolX',toldirect2,'Algorithm','sqp','Display','iter',...
        'MaxFunctionEvaluations',2000,'MaxSQPIter', 1000,'SpecifyConstraintGradient',false,'HonorBounds',true); 
