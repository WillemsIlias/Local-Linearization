...
if ~isempty(theta_fminimax)
    if (parallel && BCS_EAM ~= 1) || (BCS_EAM ==1 && LL_EAM == 1)
        ...
        CV_maxviol = 10*ones(num_fminimax,1);
        CV_fminimax = 10*ones(num_fminimax,1);
        for ll = 1:num_fminimax
            [~,CV_fminimax(ll),theta_feas,CV_maxviol(ll)] = KMS_31_Estep(theta_fminimax(ll,:),f_ineq,f_eq,f_ineq_keep,f_eq_keep,f_stdev_ineq,f_stdev_eq,G_ineq,G_eq,KMSoptions);
            if CV_maxviol(ll) <= CVtol
                flag_feas = 1;
                break;
            end
        end
