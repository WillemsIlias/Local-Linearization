...
% Run BCS (if required)
name     = strcat('KMS_samplesize',num2str(n),'_dX',num2str(dX),'_multi',num2str(KMSoptions.EI_multi_num),'_simlo',num2str(sim_lo));
...
for mm = sim_lo:sim_hi
    ...
    t_KMS(mm) = toc(t2);
    disp('remaining days')
    disp((sim_hi-mm)*mean(t_KMS(sim_lo:mm))/3600/24)
